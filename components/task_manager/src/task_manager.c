...
#if (TASK_MANAGER_ENABLE_MCP == 1)
#include "esp_mcp_data.h"
#include "esp_mcp_engine.h"
#include "esp_mcp_property.h"
#include "esp_mcp_tool.h"
#endif /* TASK_MANAGER_ENABLE_MCP == 1 */
...
int task_manager_create_task(const char *name, int type, int duration_minutes)
{
    ...
    s_tasks[task_id].timer = xTimerCreate("task_timer", pdMS_TO_TICKS(1000), pdTRUE, /* auto-reload */
                                          (void *)(intptr_t)task_id, task_timer_callback);
    ...
    ESP_LOGI(TAG, "Task created: \"%s\" (id=%d, type=%d, duration=%d min)", name, task_id, type, duration_minutes);
...
esp_err_t task_manager_pause_task(int task_id)
{
    ...
    ESP_LOGI(TAG, "Task paused: \"%s\" (id=%d, remaining=%d sec)", s_tasks[task_id].info.name, task_id,
             s_tasks[task_id].info.remaining_seconds);
...
esp_err_t task_manager_resume_task(int task_id)
{
    ...
    ESP_LOGI(TAG, "Task resumed: \"%s\" (id=%d, remaining=%d sec)", s_tasks[task_id].info.name, task_id,
             s_tasks[task_id].info.remaining_seconds);
...
esp_err_t task_manager_stop_task(int task_id)
{
    ...
    ESP_LOGI(TAG, "Task stopped: \"%s\" (id=%d, prev_state=%d)", s_tasks[task_id].info.name, task_id, prev_state);
...
esp_err_t task_manager_stop_all_tasks(void)
{
    ...
    for (int i = 0; i < TASK_MANAGER_MAX_TASKS; i++) {
        if (s_tasks[i].in_use &&
            (s_tasks[i].info.state == TASK_STATE_RUNNING || s_tasks[i].info.state == TASK_STATE_PAUSED ||
...
static esp_mcp_value_t mcp_tool_task_create(const esp_mcp_property_list_t *properties)
{
    ...
    ESP_LOGI(TAG, "[MCP] self.task.create: name=\"%s\" type=%d duration=%d min", name ? name : "null", type,
             duration_minutes);
...
static esp_mcp_value_t mcp_tool_task_list(const esp_mcp_property_list_t *properties)
{
    ...
    for (int i = 0; i < TASK_MANAGER_MAX_TASKS; i++) {
        ...
        offset += snprintf(json_buf + offset, sizeof(json_buf) - offset,
                           "{\"id\":%d,\"name\":\"%s\",\"type\":\"%s\",\"duration_min\":%d,\"remaining\":\"%d:%02d\","
                           "\"state\":\"%s\"}",
                           s_tasks[i].info.id, s_tasks[i].info.name, type_str, s_tasks[i].info.duration_minutes,
                           remaining_min, remaining_sec, state_str);
...
esp_err_t task_manager_register_mcp_tools(esp_mcp_t *mcp)
{
    ...
    esp_mcp_tool_t *create_tool = esp_mcp_tool_create(
        "self.task.create", "创建一个新任务（计时器），支持专注、学习、工作和自定义类型", mcp_tool_task_create);
    if (!create_tool) {
    ...
    /* self.task.stop */
    esp_mcp_tool_t *stop_tool = esp_mcp_tool_create("self.task.stop", "停止一个任务", mcp_tool_task_stop);
    if (!stop_tool) {
    ...
    /* self.task.stop_all */
    esp_mcp_tool_t *stop_all_tool = esp_mcp_tool_create("self.task.stop_all", "停止所有任务", mcp_tool_task_stop_all);
    if (!stop_all_tool) {
    ...
    /* self.task.list */
    esp_mcp_tool_t *list_tool = esp_mcp_tool_create("self.task.list", "列出所有任务及其状态", mcp_tool_task_list);
    if (!list_tool) {
...
#else /* TASK_MANAGER_ENABLE_MCP == 0 */

/* MCP tools disabled: provide stub implementation */
esp_err_t task_manager_register_mcp_tools(esp_mcp_t *mcp)
{
    (void)mcp;
    ESP_LOGW(TAG, "Task MCP tools disabled (TASK_MANAGER_ENABLE_MCP=0)");
    return ESP_ERR_NOT_SUPPORTED;
}

#endif /* TASK_MANAGER_ENABLE_MCP */
