...
static void heartbeat_timer_callback(void *arg)
{
    ...
    for (int i = 0; i < s_client_count; i++) {
        /* Check for timeout */
        if (s_client_last_pong_ms[i] > 0 &&
            (now_ms - s_client_last_pong_ms[i]) > (int64_t)s_timeout_sec * 1000) {
            ESP_LOGW(TAG, "Client fd=%d heartbeat timeout (last pong %llds ago)",
                     s_client_fds[i],
...
esp_err_t heartbeat_service_start_server(int ping_interval_sec, int timeout_sec)
{
    ...
    s_running = true;
    ESP_LOGI(TAG, "Heartbeat service started (ping=%ds, timeout=%ds)",
             s_ping_interval_sec, s_timeout_sec);
    return ESP_OK;
}
