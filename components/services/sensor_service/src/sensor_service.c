 ...
 *   1. Accumulate raw samples over 10-second windows (I: integration)
 *   2. Compute block average and apply EMA low-pass filter
 *   3. Publish smoothed value once per minute
 *
 *   - P: proportional tracking of each 10s block average
 *   - I: integration via block averaging over 10s windows
 *   - D: damping via EMA (exponential moving average)
 */

/* --- 10-second block accumulation --- */
#define SAMPLES_PER_BLOCK      100   /* 100ms interval × 100 = 10s */
#define BLOCKS_PER_MINUTE        6   /* 6 × 10s = 60s */

/* EMA filter coefficient (0.0 – 1.0, lower = smoother) */
#define LIGHT_EMA_ALPHA        0.10f
...
/* Per-minute block-average buffer (for debug/query) */
...
void sensor_service_feed_sample(float lux)
{
    ...
    if (s_block_count >= SAMPLES_PER_BLOCK) {
        ...
        /* Every BLOCKS_PER_MINUTE blocks → publish once per minute */
        if (s_block_idx == 0) {
            float minute_avg = 0.0f;
            for (int i = 0; i < BLOCKS_PER_MINUTE; i++) {
                minute_avg += s_block_ring[i];
            }
            minute_avg /= (float)BLOCKS_PER_MINUTE;

            uint8_t level = sensor_service_lux_to_level(s_smoothed_lux);
            ESP_LOGI(TAG, "Ambient light: %.0f lux (min-avg %.0f, level %d)  [PID-smoothed]",
                     (double)s_smoothed_lux, (double)minute_avg, level);
