...
static const char *s_welcome_messages[] = {
    "嗨~主人，想我了吗？Focus一直都在哦",
    "哈哈哈，主人，你摸的我脑袋痒痒的，好像有什么长出来了~",
};
#define WELCOME_MSG_COUNT 2
...
static void gesture_callback(const touch_gesture_event_t *event, void *ctx)
{
    ...
    case TOUCH_GESTURE_TAP: {
        ESP_LOGI(TAG, "TAP detected - triggering welcome sequence");

        /* 1. Turn on head LED at current brightness level */
#if CONFIG_EXAMPLE_ENABLE_LED
        if (led_is_initialized()) {
            uint8_t target_brightness = s_brightness_levels[s_brightness_level_idx];
            if (target_brightness == 0) {
                target_brightness = s_brightness_levels[BRIGHTNESS_DEFAULT_LEVEL];
            }
        ...
        /* 4. Log welcome message (dowm will poll /api/touch/wake and trigger xiaozhi TTS) */
