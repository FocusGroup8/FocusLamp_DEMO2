...
static void draw_ball(simple_gui_t *gui, ball_state_t *ball)
{
    ...
    // Draw rotation indicator: a white pie slice (~45°) that rotates with the ball.
    // This is far more visible than a thin line on a solid-color ball.
    gui_draw_filled_pie(gui, ball->x, ball->y, ball->radius, ball->rotation, M_PI / 4, COLOR_WHITE);

    // Draw ball outline (circle)
    gui_draw_rect_outline(gui, ball->x - ball->radius, ball->y - ball->radius, ball->x + ball->radius,
                          ball->y + ball->radius, COLOR_WHITE, 1);
}

// Gesture callback function for game control
static ball_state_t *game_ball                 = NULL;
static gesture_recognition_t *game_gesture_ctx = NULL;
...
static void game_gesture_callback(gesture_type_t gesture, void *user_data)
{
    ...
    case GESTURE_PINCH_OUT: {
        if (game_ball->radius < APP_GAME_BALL_MAX_RADIUS) {
            game_ball->radius += APP_GAME_BALL_RADIUS_STEP;
            game_ball->score += APP_GAME_SCORE_PINCH;
            // Clamp ball position to new (smaller) valid range
            const int nx_max = APP_GAME_CLEAR_X2 - game_ball->radius;
            const int ny_max = APP_GAME_CLEAR_Y2 - game_ball->radius;
            const int nx_min = APP_GAME_CLEAR_X1 + game_ball->radius;
            const int ny_min = APP_GAME_CLEAR_Y1 + game_ball->radius;
            if (game_ball->x > nx_max)
                game_ball->x = nx_max;
            if (game_ball->x < nx_min)
                game_ball->x = nx_min;
            if (game_ball->y > ny_max)
                game_ball->y = ny_max;
            if (game_ball->y < ny_min)
                game_ball->y = ny_min;
    ...
    case GESTURE_ROTATION: {
        // Use signed rotation from gesture context: +CW / -CCW (degrees → radians)
        float delta_deg = 0;
        if (game_gesture_ctx) {
            delta_deg = game_gesture_ctx->last_rotation_signed;
        }
        if (delta_deg == 0) {
            delta_deg = APP_GAME_BALL_ROTATION_STEP_DEG; // fallback
        }
        float delta_rad = delta_deg * (float)M_PI / 180.0f;
        game_ball->rotation += delta_rad;
        if (game_ball->rotation < 0) {
            game_ball->rotation += 2 * (float)M_PI;
        }
        if (game_ball->rotation >= 2 * (float)M_PI) {
            game_ball->rotation -= 2 * (float)M_PI;
        }
        game_ball->score += APP_GAME_SCORE_ROTATION;
        ESP_LOGI(TAG, "Ball rotated: delta=%.1f deg, angle=%.1f deg, score=%d", delta_deg,
                 game_ball->rotation * 180 / M_PI, game_ball->score);
        break;
    }
...
void touch_game_demo(simple_gui_t *gui, esp_lcd_touch_handle_t tp)
{
    ...
    ball_state_t ball = {.x          = APP_GAME_BALL_INIT_X,
    ...
    game_gesture_ctx = &gesture_ctx;
    gesture_recognition_init(&gesture_ctx, game_gesture_callback, &gesture_ctx);

    // Draw static background (title + instructions + game area outline) to BOTH
    // frame buffers. In double-buffer mode, each fb must have the static content
    // pre-rendered so that per-frame updates only touch the dynamic regions.
    for (int i = 0; i < 2; i++) {
        ...
        gui_draw_rect_outline(gui, APP_GAME_AREA_OUTLINE_X1, APP_GAME_AREA_OUTLINE_Y1, APP_GAME_AREA_OUTLINE_X2,
                              APP_GAME_AREA_OUTLINE_Y2, COLOR_GREEN, 2);
        gui_draw_string(gui, 10, 175, "Game Area", COLOR_GREEN, COLOR_BLACK, 1);
        gui_swap_buffers(gui); // present this fb, next iteration draws to the other
    }
    ...
    while (frame_count < APP_GAME_MAX_FRAMES) {
        ...
        if (!ball.paused) {
            ...
            // Dynamic boundary checking: ball center limits depend on current radius
            // so that the ball edge never exceeds the clear area
            // (clear area: X∈[15,465], Y∈[185,395])
            const int x_min = APP_GAME_CLEAR_X1 + ball.radius;
            const int x_max = APP_GAME_CLEAR_X2 - ball.radius;
            const int y_min = APP_GAME_CLEAR_Y1 + ball.radius;
            const int y_max = APP_GAME_CLEAR_Y2 - ball.radius;
            ...
            // Rotation only changes via gesture callback (no auto-rotation)
        }

        // Clear game area
        gui_draw_filled_rect(gui, APP_GAME_CLEAR_X1, APP_GAME_CLEAR_Y1, APP_GAME_CLEAR_X2, APP_GAME_CLEAR_Y2,
                             COLOR_BLACK);

        // Draw ball
        draw_ball(gui, &ball);

        // Clear info text area (prevents residual text from previous frames)
        gui_draw_filled_rect(gui, 0, APP_GAME_INFO_Y1, gui->width - 1, APP_GAME_INFO_Y2, COLOR_BLACK);

        // Draw score
        char score_str[32];
        sprintf(score_str, "Score: %d", ball.score);
        gui_draw_string(gui, 10, APP_GAME_INFO_SCORE_Y, score_str, COLOR_WHITE, COLOR_BLACK, 2);

        // Draw pause indicator
        if (ball.paused) {
            gui_draw_string(gui, 10, APP_GAME_INFO_PAUSED_Y, "PAUSED", COLOR_RED, COLOR_BLACK, 2);
        }
        ...
        if (ball.velocity_x != 0 || ball.velocity_y != 0) {
            ...
            gui_draw_string(gui, 10, APP_GAME_INFO_VELOCITY_Y, vel_str, ball.color, COLOR_BLACK, 1);
        }

        // Present the drawn frame (double-buffer flip)
    ...
    // Game complete message — draw to both fbs and present
    for (int i = 0; i < 2; i++) {
        gui_clear_screen(gui, COLOR_BLACK);
