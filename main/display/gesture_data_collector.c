...
void gesture_data_collector_demo(simple_gui_t *gui, esp_lcd_touch_handle_t tp)
{
    ...
    while (gesture_count < max_gestures) {
        ...
        if (ret == ESP_OK && touch_cnt > 0) {
            ...
            if (tracking && touch_cnt == 1) {
                ...
                if (current_time - last_log_time >= 200) {
                    ...
                    ESP_LOGI(TAG, "  Time: %lu ms, Pos: (%d,%d), Dist: %.1f px, Vel: %.3f px/ms", current_duration,
                             current_x, current_y, current_distance, velocity);
        ...
        if (tracking && touch_cnt == 0) {
            ...
            ESP_LOGI(TAG, "  Distance threshold: 50 px (result: %s)", final_distance >= 50.0f ? "PASS" : "FAIL");
            ESP_LOGI(TAG, "  Velocity threshold: 0.05 px/ms (result: %s)", final_velocity >= 0.05f ? "PASS" : "FAIL");
            ESP_LOGI(TAG, "  Combined result: %s",
                     (final_distance >= 50.0f && final_velocity >= 0.05f) ? "GESTURE DETECTED"
            ...
            gui_draw_string(gui, 60, 340, result_str,
                            (final_distance >= 50.0f && final_velocity >= 0.05f) ? COLOR_GREEN : COLOR_RED, COLOR_BLACK,
                            2);
