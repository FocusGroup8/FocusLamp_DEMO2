...
### 5.3 灯头灯亮度（固定五档，占空比 5%~30%）

PWM 占空比满量程为 100%（`LED_MAX_DUTY_PERCENT=100`），五档占空比由 5% 线性递增至 30%。
...
- **单击灯头亮灯**：按当前档位占空比点亮（`s_brightness_levels[current]`）
- **双击调档**：档位+1（`{5,11,18,24,30}`）循环切换，NVS 保存
- **brightness↔占空比**：灯头板 `percent_to_duty()` 将亮度百分比直接映射为 PWM 占空比（duty = percent × 1023 / 100），基座板 `/api/led/on` 的 `brightness` 字段即占空比百分比
