 ...
 *
 * Bridges esp_xiaozhi's audio_callback (OPUS-encoded TTS data) to I2S hardware
 * playback. Also provides a microphone recording interface for future voice
 * input.
 *
 ...
 * allocation and internal DRAM stack. This ensures PIE coprocessor (SIMD)
 * context is properly managed by FreeRTOS lazy context switching, preventing
 * MCAUSE 0x1f exceptions that occurred when decoding ran in the WebSocket
 * client task context.
 */
 ...
 * bottom). ESP32-P4: FPU=136B, PIE=~200B, HWLOOP=~20B. With 8KB stack the
 * coprocessor areas fit comfortably. */
...
#define MALLOC_DISABLE_EXTERNAL_ALLOCS_NEG1 ((size_t)-1)
  ...
  s_opus_out_buf = heap_caps_malloc(OPUS_DEC_OUT_BUF_SIZE,
                                    MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (s_opus_out_buf == NULL) {
    ESP_LOGE(TAG,
             "Failed to allocate OPUS output buffer (%d bytes) in internal RAM",
             OPUS_DEC_OUT_BUF_SIZE);
  ...
  s_pcm32_buf =
      heap_caps_malloc(PCM32_BUF_SIZE, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (s_pcm32_buf == NULL) {
    ESP_LOGE(TAG, "Failed to allocate PCM32 buffer (%d bytes) in internal RAM",
             (int)PCM32_BUF_SIZE);
 ...
 *-------------------------------------------------------------*/
static esp_err_t write_pcm_to_i2s(const int16_t *pcm_data,
                                  size_t sample_count) {
  /* 使用预分配的静态缓冲区，避免每次TTS回调都malloc/free */
  if (sample_count > 960) {
    ESP_LOGW(TAG, "Sample count %d exceeds buffer size, truncating",
             (int)sample_count);
    sample_count = 960;
  }
  ...
  /* I2S DMA buffer = 16 desc × 960 samples × 4 bytes = 61440 bytes ≈ 960ms of
   * audio. When buffer is nearly full (e.g., during TTS burst), write needs to
   * wait for the oldest buffer to drain. Use 2000ms timeout (2× buffer time) to
   * avoid ESP_ERR_TIMEOUT during TTS playback. Previous 500ms was too short. */
  esp_err_t ret = i2s_channel_write(s_tx_handle, s_pcm32_buf, buf32_size,
...
static void audio_decode_task(void *arg) {
  ...
  while (s_decode_task_running) {
    ...
    if (xQueueReceive(s_decode_queue, &opus_data, pdMS_TO_TICKS(1000)) !=
        pdTRUE) {
      continue;
    }
    ...
    if (s_opus_decoder == NULL) {
      ...
      if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to create OPUS decoder, discarding %d bytes",
                 data_len);
        free(opus_data);
        opus_data = NULL;
        continue;
    ...
    if (s_opus_out_buf == NULL) {
      ESP_LOGD(TAG, "OPUS output buffer not ready, discarding %d bytes",
               data_len);
      free(opus_data);
      opus_data = NULL;
      continue;
    ...
    esp_audio_err_t ret =
        esp_opus_dec_decode(s_opus_decoder, &raw, &frame, &dec_info);
    ...
    ESP_LOGD(TAG, "OPUS decoded: %d bytes → %d samples (%dHz, %dch)",
             frame.decoded_size, sample_count, dec_info.sample_rate,
             dec_info.channel);
...
static esp_err_t decode_task_start(void) {
  ...
  s_decode_task_handle = xTaskCreateStaticPinnedToCore(
      audio_decode_task, DECODE_TASK_NAME, DECODE_TASK_STACK_SIZE,
      NULL, /* task parameter */
      DECODE_TASK_PRIORITY, s_decode_task_stack, &s_decode_task_tcb, core);
...
esp_err_t audio_bridge_init(const audio_bridge_config_t *config) {
  ...
  ESP_LOGI(TAG, "I2S channel pair created (TX=%p, RX=%p)", s_tx_handle,
           s_rx_handle);
  ...
  ESP_LOGI(TAG, "Config: PHILIPS, MONO, LEFT, %dHz, 32-bit",
           config->sample_rate);
  ESP_LOGI(TAG, "GPIO: BCLK=%d, WS=%d, DOUT=%d, DIN=%d", config->bclk_gpio,
           config->ws_gpio, config->dout_gpio, config->din_gpio);
  ...
  while (preload_bytes == sizeof(preload_buf)) {
    ret = i2s_channel_preload_data(s_tx_handle, preload_buf,
                                   sizeof(preload_buf), &preload_bytes);
    if (ret != ESP_OK) {
      break;
    }
    preload_count++;
  ...
  s_initialized = true;
  ESP_LOGI(TAG,
           "Audio bridge initialized successfully (dedicated decode task)");
  return ESP_OK;
...
esp_err_t audio_bridge_write_pcm(const void *data, size_t len,
  ...
  return i2s_channel_write(s_tx_handle, data, len, bytes_written,
                           ticks_to_wait);
}
...
void audio_bridge_tts_callback(const uint8_t *data, int len, void *ctx) {
   ...
   * This runs in the WebSocket client task context (lightweight — no PIE/FPU
   * here). Format: [2 bytes data_len (little-endian)] [data_len bytes OPUS
   * frame data] The decode task will read the length prefix and decode in its
   * own context. */
  size_t buf_size = 2 + (size_t)len;
  uint8_t *opus_buf = malloc(buf_size);
  if (opus_buf == NULL) {
    ESP_LOGW(TAG, "Failed to allocate %d bytes for OPUS frame, discarding",
             (int)buf_size);
    return;
  }
  ...
  if (xQueueSend(s_decode_queue, &opus_buf, 0) != pdTRUE) {
    ...
    if (xQueueReceive(s_decode_queue, &old_buf, 0) == pdTRUE &&
        old_buf != NULL) {
      free(old_buf);
    }
    if (xQueueSend(s_decode_queue, &opus_buf, 0) != pdTRUE) {
...
void audio_bridge_flush_tts(void) {
  ...
  /* Drain all pending OPUS frames from the decode queue.
   * Called when TTS stops to prevent stale frames from causing
   * unnecessary I2S writes (and timeouts) after playback has ended. */
  int flushed = 0;
  uint8_t *opus_buf = NULL;
  while (xQueueReceive(s_decode_queue, &opus_buf, 0) == pdTRUE) {
    if (opus_buf != NULL) {
      free(opus_buf);
      opus_buf = NULL;
      flushed++;
  ...
  if (flushed > 0) {
    ESP_LOGI(TAG, "Flushed %d stale OPUS frame(s) from decode queue", flushed);
  }
}
...
int audio_bridge_get_volume(void) { return s_volume; }
...
static esp_err_t opus_encoder_create(void) {
  ...
  ESP_LOGI(
      TAG, "OPUS encoder create: internal free=%u, PSRAM free=%u",
  ...
  if (ret != ESP_AUDIO_ERR_OK || s_opus_encoder == NULL) {
    ESP_LOGE(
        TAG,
        "Failed to open OPUS encoder: %d (internal free=%u, PSRAM free=%u)",
        ret,
...
static void mic_task(void *arg) {
  ...
  int32_t *i2s_rx_buf = heap_caps_malloc(MIC_I2S_READ_SIZE,
                                         MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  int16_t *pcm16_buf = heap_caps_malloc(MIC_PCM16_BUF_SIZE,
                                        MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  uint8_t *opus_out_buf = heap_caps_malloc(
      MIC_OPUS_OUT_BUF_SIZE, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  ...
  while (s_mic_task_running) {
    ...
    if (ret != ESP_OK) {
      ESP_LOGW(TAG, "I2S RX read failed: %s (bytes_read=%u)",
    ...
    if (bytes_read < MIC_I2S_READ_SIZE) {
      ...
      ESP_LOGW(TAG, "I2S RX partial read: %u/%u bytes", (unsigned)bytes_read,
               (unsigned)MIC_I2S_READ_SIZE);
      continue;
    }
    ...
    if (frame_count <= 3 || (frame_count % 50 == 0)) {
      ...
      ESP_LOGI(TAG,
               "Mic frame #%d: i2s[0..3]=%08X,%08X,%08X,%08X "
               "pcm[0..3]=%d,%d,%d,%d peak=%ld",
               frame_count, (unsigned)i2s_rx_buf[0], (unsigned)i2s_rx_buf[1],
    ...
    esp_audio_err_t enc_ret =
        esp_opus_enc_process(s_opus_encoder, &in_frame, &out_frame);
    if (enc_ret != ESP_AUDIO_ERR_OK) {
      ESP_LOGW(TAG, "OPUS encode failed: %d", enc_ret);
      continue;
    }

    /* Get encoded output size from out_frame.encoded_bytes (NOT .len — .len is
...
mic_cleanup:
  ...
  if (pcm16_buf)
    heap_caps_free(pcm16_buf);
  if (opus_out_buf)
    heap_caps_free(opus_out_buf);
...
esp_err_t audio_bridge_mic_start(audio_bridge_mic_callback_t callback,
  ...
  size_t test_bytes = 0;
  for (int i = 0; i < 3; i++) {
    ret = i2s_channel_read(s_rx_handle, test_buf, sizeof(test_buf), &test_bytes,
                           pdMS_TO_TICKS(100));
    ESP_LOGI(TAG, "RX test read %d: %s (%u bytes)", i + 1, esp_err_to_name(ret),
             (unsigned)test_bytes);
    if (ret != ESP_OK) {
  ...
  s_mic_task_handle = xTaskCreateStaticPinnedToCore(
      mic_task, MIC_TASK_NAME, MIC_TASK_STACK_SIZE, NULL, MIC_TASK_PRIORITY,
      s_mic_task_stack, &s_mic_task_tcb, core);
...
esp_err_t audio_bridge_mic_stop(void) {
   ...
   * and exit its main loop. The previous order (delay -> NULL handle ->
   * disable) left the task blocked in i2s_channel_read for up to 1000ms, then
...
int audio_bridge_read_ref_pcm(int16_t *out_buf, int samples,
  ...
  char *item = (char *)xRingbufferReceiveUpTo(
      s_ref_ringbuf, &item_size, pdMS_TO_TICKS(timeout_ms), needed);
  ...
  if (item != NULL && item_size > 0) {
    ...
    if (samples_read < samples) {
      memset(&out_buf[samples_read], 0,
             (samples - samples_read) * sizeof(int16_t));
    }
    return samples_read;
  }
...
esp_err_t audio_bridge_deinit(void) { return ESP_ERR_NOT_SUPPORTED; }
...
int audio_bridge_get_volume(void) { return 0; }
...
esp_err_t audio_bridge_mic_stop(void) { return ESP_ERR_NOT_SUPPORTED; }
