...
esp_err_t message_queue_init(int max_pending_msgs, int max_retries)
{
    ...
    ESP_LOGI(TAG, "Message queue initialized (max=%d, retries=%d)",
             s_max_pending, s_max_retries);
    return ESP_OK;
}
...
esp_err_t message_queue_enqueue(int client_fd, const char *data, int len, msg_queue_type_t type)
{
    ...
    ESP_LOGD(TAG, "Queued message for fd=%d (type=%s, len=%d, queue=%d/%d)",
             client_fd, type == MSG_QUEUE_TYPE_TEXT ? "TEXT" : "BIN",
             len, s_entry_count, s_max_pending);
...
esp_err_t message_queue_process_pending(void)
{
    ...
    for (int i = 0; i < s_max_pending; i++) {
        ...
        if (s_entries[i].type == MSG_QUEUE_TYPE_TEXT) {
            send_ret = ws_manager_server_send_text(s_entries[i].client_fd,
                                                    s_entries[i].data, s_entries[i].len);
        } else {
            send_ret = ws_manager_server_send_binary(s_entries[i].client_fd,
                                                      s_entries[i].data, s_entries[i].len);
        }
        ...
        if (send_ret == ESP_OK) {
            ...
            if (s_entries[i].retry_count >= s_max_retries) {
                ESP_LOGW(TAG, "Dropping message for fd=%d after %d retries",
                         s_entries[i].client_fd, s_entries[i].retry_count);
                free(s_entries[i].data);
                memset(&s_entries[i], 0, sizeof(msg_entry_t));
                s_entry_count--;
            }
