...
esp_err_t focuslamp_bridge_register_mcp_tools(esp_mcp_t *mcp) {
  ...
  /* LED tools (5) */
  esp_mcp_tool_t *t;
  t = esp_mcp_tool_create("self.focuslamp.led.on", "开启FocusLamp LED灯(可指定亮度0-100)", mcp_tool_led_on);
  ...
  t = esp_mcp_tool_create("self.focuslamp.led.off", "关闭FocusLamp LED灯", mcp_tool_led_off);
  if (t) esp_mcp_add_tool(mcp, t);

  t = esp_mcp_tool_create("self.focuslamp.led.set_brightness", "设置FocusLamp LED亮度(0-100)", mcp_tool_led_set_brightness);
  if (t) { esp_mcp_property_t *p = esp_mcp_property_create_with_range("brightness", 0, 100); esp_mcp_tool_add_property(t, p); esp_mcp_add_tool(mcp, t); }

  t = esp_mcp_tool_create("self.focuslamp.led.set_effect", "设置FocusLamp LED效果(steady/breathing/rainbow/blinking)", mcp_tool_led_set_effect);
  ...
  t = esp_mcp_tool_create("self.focuslamp.led.set_color", "设置FocusLamp LED颜色(r/g/b各0-255)", mcp_tool_led_set_color);
  if (t) {
  ...
  /* Display tools (3) */
  t = esp_mcp_tool_create("self.focuslamp.display.on", "开启FocusLamp显示屏", mcp_tool_display_on);
  if (t) esp_mcp_add_tool(mcp, t);

  t = esp_mcp_tool_create("self.focuslamp.display.off", "关闭FocusLamp显示屏", mcp_tool_display_off);
  if (t) esp_mcp_add_tool(mcp, t);

  t = esp_mcp_tool_create("self.focuslamp.display.set_brightness", "设置FocusLamp显示屏亮度(0-255)", mcp_tool_display_set_brightness);
  ...
  /* LCD tools (4) */
  t = esp_mcp_tool_create("self.focuslamp.lcd.next_page", "切换FocusLamp LCD下一页", mcp_tool_lcd_next_page);
  if (t) esp_mcp_add_tool(mcp, t);

  t = esp_mcp_tool_create("self.focuslamp.lcd.set_expression", "设置FocusLamp LCD表情(normal/happy/sad/angry/surprised/sleepy)", mcp_tool_lcd_set_expression);
  ...
  t = esp_mcp_tool_create("self.focuslamp.lcd.enable_blink", "启用FocusLamp LCD自动眨眼", mcp_tool_lcd_enable_blink);
  if (t) esp_mcp_add_tool(mcp, t);

  t = esp_mcp_tool_create("self.focuslamp.lcd.disable_blink", "禁用FocusLamp LCD自动眨眼", mcp_tool_lcd_disable_blink);
  if (t) esp_mcp_add_tool(mcp, t);

  /* Servo tools (2) */
  t = esp_mcp_tool_create("self.focuslamp.servo.set_position", "设置FocusLamp舵机位置(servo_id:0=EM3 0-3000, 1-4=LX 0-1000)", mcp_tool_servo_set_position);
  if (t) {
  ...
  t = esp_mcp_tool_create("self.focuslamp.servo.go_home", "FocusLamp所有舵机归零(可选time_ms 0-65535)", mcp_tool_servo_go_home);
  ...
  ESP_LOGI(TAG, "FocusLamp bridge MCP tools registered (14 tools)");
  return ESP_OK;
}
