...
static esp_err_t cmd_head_light_on(void);
static esp_err_t cmd_head_light_off(void);
static esp_err_t cmd_base_light_on(void);
static esp_err_t cmd_base_light_off(void);
static esp_err_t cmd_expression_happy(void);
static esp_err_t cmd_expression_normal(void);
static esp_err_t cmd_expression_surprised(void);

/* Command keyword table */
static const voice_command_t s_commands[] = {
    /* Light control - base (底座灯) */
    { "open light",      cmd_light_on        },
    { "turn on light",   cmd_light_on        },
    { "打开底灯",         cmd_base_light_on   },
    { "开底灯",           cmd_base_light_on   },
    { "close light",     cmd_light_off       },
    { "turn off light",  cmd_light_off       },
    { "关闭底灯",         cmd_base_light_off  },
    { "关底灯",           cmd_base_light_off  },
    /* Light control - head (灯头灯) */
    { "打开灯头",         cmd_head_light_on   },
    { "开灯头",           cmd_head_light_on   },
    { "关闭灯头",         cmd_head_light_off  },
    { "关灯头",           cmd_head_light_off  },
    /* Brightness */
    { "brightness up",   cmd_brightness_up   },
    { "brighter",        cmd_brightness_up   },
    { "brightness down", cmd_brightness_down },
    { "dimmer",          cmd_brightness_down },
    { "调亮",             cmd_brightness_up   },
    { "调暗",             cmd_brightness_down },
    /* Color temperature */
    { "warm light",      cmd_colortemp_warm  },
    { "cool light",      cmd_colortemp_cool  },
    { "暖光",             cmd_colortemp_warm  },
    { "冷光",             cmd_colortemp_cool  },
    /* Mode switching */
    { "focus mode",      cmd_mode_focus      },
    { "companion mode",  cmd_mode_companion  },
    { "music mode",      cmd_mode_music      },
    { "开启专注模式",     cmd_mode_focus      },
    { "专注模式",         cmd_mode_focus      },
    { "开启陪伴模式",     cmd_mode_companion  },
    { "陪伴模式",         cmd_mode_companion  },
    /* Expression switching (大屏幕表情) */
    { "开心表情",         cmd_expression_happy    },
    { "正常表情",         cmd_expression_normal   },
    { "惊讶表情",         cmd_expression_surprised},
    /* Arm actions */
    { "wave",            cmd_arm_wave        },
...
static esp_err_t cmd_head_light_on(void)
{
    ...
    ESP_LOGI(TAG, "Command: head light on (ambient-adjusted)");
    return ESP_OK;
}
...
{
    lamp_head_led_off();
    ESP_LOGI(TAG, "Command: head light off");
    return ESP_OK;
}
...
{
    led_service_set_brightness(200);
    ESP_LOGI(TAG, "Command: base light on");
    return ESP_OK;
}
...
{
    led_service_set_brightness(0);
    ESP_LOGI(TAG, "Command: base light off");
    return ESP_OK;
}
...
static esp_err_t cmd_expression_happy(void)
{
    ...
    ESP_LOGI(TAG, "Command: expression happy");
    return ESP_OK;
}
...
static esp_err_t cmd_expression_normal(void)
{
    ...
    ESP_LOGI(TAG, "Command: expression normal");
    return ESP_OK;
}

static esp_err_t cmd_expression_surprised(void)
{
    lamp_head_set_expression("surprised");
    ESP_LOGI(TAG, "Command: expression surprised");
    return ESP_OK;
}
