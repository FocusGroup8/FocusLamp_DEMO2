...
void app_main(void)
{
    ...
    if (err == ESP_OK) {
        ...
        /* Initialize TTS HTTP API (POST /api/tts/speak) */
        err = tts_http_api_init();
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "TTS HTTP API initialized (POST /api/tts/speak)");
        } else {
            ESP_LOGW(TAG, "TTS HTTP API init failed: %s", esp_err_to_name(err));
        }
    }
#endif
