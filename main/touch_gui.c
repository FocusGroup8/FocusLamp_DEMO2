...
{
    uint32_t bg_color = (button->state == BUTTON_STATE_PRESSED) ? 
                        button->color_pressed : button->color_idle;
    
    // Draw button background
    gui_draw_filled_rect(gui, button->x, button->y,
                         button->x + button->width - 1,
                         button->y + button->height - 1, bg_color);
    
    // Draw button border
    gui_draw_rect_outline(gui, button->x, button->y,
                          button->x + button->width - 1,
    ...
    if (button->text) {
        ...
        gui_draw_string(gui, text_x, text_y, button->text,
                       button->text_color, bg_color, button->text_size);
    }
}

bool touch_button_check_hit(touch_button_t *button, uint16_t touch_x, uint16_t touch_y)
{
    return (touch_x >= button->x && 
    ...
    if (touch_active && touch_button_check_hit(button, touch_x, touch_y)) {
        ...
        if (button->state == BUTTON_STATE_IDLE) {
            ...
            if (current_time - button->last_press_time >= button->debounce_ms) {
                ...
                state_changed = true;
                ESP_LOGI(TAG, "Button '%s' pressed at (%d, %d)", 
                         button->text ? button->text : "unnamed", touch_x, touch_y);
            }
        }
    ...
    while (frame_count < 600) {  // Run for ~60 seconds
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (button_click_count[i] != last_counts[i]) {
                ...
                gui_draw_string(gui, 10 + i * 120, 330, count_str, 
                               buttons[i].color_idle, COLOR_BLACK, 1);
