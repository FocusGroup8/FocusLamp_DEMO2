                                           ...
                                           esp_lcd_touch_handle_t tp)
{
    ...
    if (ret != ESP_OK || touch_cnt == 0) {
        ...
        if (gesture->history_count[0] >= 2) {
            ...
            GESTURE_LOGD("Swipe check: start=(%d,%d), end=(%d,%d), duration=%lu",
                         start_x, start_y, end_x, end_y, end_time - start_time);
        ...
        if (distance >= gesture->swipe_threshold && velocity >= 0.01f) {
                    ...
                    ESP_LOGI(TAG, "Swipe detected: %s, distance: %.1f px, velocity: %.2f px/ms",
                             ...
                             gesture->current_gesture == GESTURE_SWIPE_UP ? "UP" : "DOWN",
                             distance, velocity);
            ...
            if (gesture->current_distance > 0) {
                ...
                if (scale_change >= gesture->pinch_threshold) {
                    ...
                    ESP_LOGI(TAG, "Pinch detected: %s, scale: %.2f",
                             gesture->current_gesture == GESTURE_PINCH_IN ? "IN" : "OUT",
                             scale);
        ...
        if (gesture->initial_angle != 0 && gesture->current_angle != 0) {
            float angle_change = fabs(gesture->current_angle - gesture->initial_angle);
            
            if (angle_change >= gesture->rotation_threshold) {
                gesture->current_gesture = GESTURE_ROTATION;
                
                ESP_LOGI(TAG, "Rotation detected: %.1f degrees", angle_change);
    ...
    GESTURE_LOGD("Touch active: count=%d, track_ids=[%d,%d,%d,%d,%d]",
                 touch_cnt,
                 touch_cnt > 0 ? touch_points[0].track_id : -1,
                 touch_cnt > 1 ? touch_points[1].track_id : -1,
                 touch_cnt > 2 ? touch_points[2].track_id : -1,
                 touch_cnt > 3 ? touch_points[3].track_id : -1,
    ...
    for (int i = 0; i < touch_cnt; i++) {
        ...
        if (gesture->history_count[i] < 20) {
            ...
            GESTURE_LOGD("Point %d: pos=(%d,%d), history_count=%d",
                         i, touch_points[i].x, touch_points[i].y, gesture->history_count[i]);
        }
    }
    ...
    if (touch_cnt >= 2) {
        ...
        if (gesture->initial_distance == 0) {
            ...
            ESP_LOGD(TAG, "Two-finger gesture initialized: distance=%.1f, angle=%.1f",
                     gesture->initial_distance, gesture->initial_angle);
        } else {
                                     ...
                                     uint16_t *distance, uint32_t *duration)
{
    ...
    if (gesture->history_count[0] >= 2) {
        *distance = (uint16_t)calculate_distance(
            gesture->history[0][0].x,
            gesture->history[0][0].y,
